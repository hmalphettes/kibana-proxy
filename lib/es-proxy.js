...
function proxyRequest(request, response, host, esSecure, port, user, password, getProxiedRequestPath, isUI) {
  ...
  var proxyReq = esSecure ? https.request(options) : http.request(options);
...
exports.configureESProxy = function(app, esHost, esSecure, esPort, esUser, esPassword, others) {
  app.use("/__es", function(request, response, next) {
    proxyRequest(request, response, esHost, esSecure, esPort, esUser, esPassword,
  ...
  if (others) {
    Object.keys(others).forEach(function(k) {
      var other = others[k];
      var prox = function(request, response, next) {
        proxyRequest(request, response, other.host, other.secure, other.port, other.user, other.password,
          function getProxiedRequestPath(request) {
            return request.originalUrl;
          }, true);
      };
      k.split(',').forEach(function(base) {
        app.use(base, prox);
      });
    });
  }
